...
/**
 * @package Mssql Schema class
 * @author Amadi Ifeanyi <amadiify.com>
 * 
 * This is the default Schema class for Mssql database system
 */
class Schema implements SchemaInterface, SchemaHelperInterface
{
     ...
     */
    public static $_datatypes = 'bigint,int,smallint,tinyint,bit,decimal,numeric,money,smallmoney,float,real,datetime,datetime2,date,datetimeoffset,smalldatetime,char,varchar,text,nchar,nvarchar,ntext,binary,varbinary,image,cursor,sql_variant,table,timestamp,uniqueidentifier';
    ...
    {   
        ...
        // set datatypes
        self::$datatypes = self::$_datatypes;
    ...
    {
        $this->buildQuery[] = "\t" . ' FOREIGN KEY ('.$column.') ';
    ...
    public function primary(...$arguments) : void 
    {
        ...
        $this->buildQuery[] = "\t" . $command . $arguments;
    ...
    {
        $this->buildQuery[] = "\t" .$column . ' DECIMAL('.$precision.', '.$scale.'), ';
     ...
     * @return void 
     */
    public function identity(int $seed = 1, int $increment = 1) : void 
    {
        $this->buildQuery[] = "\t" .' IDENTITY('.$seed.', '.$increment.'), ';
        $this->queryInfo['identity'] = [$seed, $increment];
    }
    ...
    {
        $this->buildQuery[] = "\t" . ' REFERENCES '.$table.' ('.$column.') ';
    ...
    public function __increment(string &$method, &$length, &$other) : void
    {
        ...
        $other = 'PRIMARY KEY IDENTITY(1,1)';
    }
    ...
    public function getSqlSavePath() : string
    {
        ...
        // return path
        return func()->const('database') . '/Sql/schema_mssql_'.md5($source).'.sql'; 
    }
			...
			foreach ($oldKeys as $index => $column) :
				...
				if (isset($newKeys[$index])) :
					...
					if ($newcolumn != $column) :
                        ...
                        // statement changed
						if ($newLine != $oldLine) $sqlArray[] = "ALTER TABLE `{$this->tableName}` ALTER COLUMN {$newcolumn} {$newcolumn} $newLine;";
			...
			foreach ($oldKeys as $index => $column) :
                ...
                if (isset($newKeys[$index]) && $this->updateSchemaFromExternal === false) :
                    ...
                    if ($newcolumn != $column) :
                        ...
                        // add to sqlarray
                        $sqlArray[] = "ALTER TABLE `{$this->tableName}` ALTER COLUMN $column $newcolumn $newLine;";
